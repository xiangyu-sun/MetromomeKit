...
public actor Metronome {
    ...
    public nonisolated let tick = PassthroughSubject<Int, Error>()
    ...
    public init(audioFormat:AVAudioFormat) {
        ...
        initiazeDefaults()
        setupEngineAndPlayer()
    }
    
    func setupEngineAndPlayer() {
    ...
    func scheduleBeats() {
        ...
        while (beatsScheduled < beatsToScheduleAhead) {
            ...
            if let buffer = soundBuffer[bufferNumber] {
                Task {
                    await self.player.scheduleBuffer(buffer, at: playerBeatTime, options: AVAudioPlayerNodeBufferOptions(rawValue: 0))
